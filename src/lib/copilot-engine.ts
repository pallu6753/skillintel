...
export function generateResponse(message: string, student: StudentFull, allSkills: string[]): string {
  ...
  switch (intent) {
    ...
    case "readiness":
      return `## 📈 Placement Readiness Analysis\n\n**AI Readiness Score: ${prediction.score}%**\n**Status: ${prediction.label}**\n\n### Score Breakdown:\n${prediction.factors.map((f) => `| ${f.name} | ${f.contribution} pts (${Math.round(f.value)}% x ${(f.weight * 100).toFixed(0)}% weight) |`).join("\n")}\n\n${prediction.score >= 70 ? "You are on track for placement! Keep building your portfolio." : prediction.score >= 50 ? "You are making progress. Focus on improving weaker areas." : "You need significant improvement in key areas."}`;
